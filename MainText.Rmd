...
which transmitted the similar asymmetric tail-dependence from drivers level ($A_S^n$ or $\alpha_S^n$)
to species level ($A_D^n$ or $\alpha_D^n$).
...
\caption{Significant Kendall correlation (based on p value from one tailed test for positive association) between $\alpha_S^n(i)$ and $\alpha_D^n(i) ; i = 1, 2, \dots 20$, for most of the selected sampling sites explains asymmetric tail-dependence between species-winter
average temperature was the reason for similar tail-dependence between $i^{th}$ species to other species.\label{fig_aphid_multipanel}}
\end{center}
\end{figure}
