...
$\{x_t^n:i=1,...,T\}$ and $\tilde{v}_t^n$ is the rank of $y_t^n$ in the set $\{y_t^n:i=1,...,T\}$. 
...
If independence could be rejected ($0.05$ significance level), we computed the partial Spearman 
correlation statistics $\cor_{l_b,u_b}$ as introduced in Ghosh et al. (in review)
at either tail of the copula. 

Given lower and upper boundaries by two parallel lines ($u_t^n+v_t^n = 2l_b$ and $u_t^n+v_t^n = 2u_b$) 
...
The sum of $\cor_{0,0.5}^n(u,v)$ and $\cor_{0.5,1}^n(u,v)$ (or some other choice of 
$\cor_{l_{b_k},u_{b_k}}^n(u,v)$ for bounds $l_{b_k},u_{b_k}$ that partition the 
interval $(0,1)$) equals the standard Spearman correlation. Higher partial Spearman correlation 
in lower tail, i. e., when $\cor_{0,0.5}^n(u,v)  - \cor_{0.5,1}^n(u,v)$ is positive, 
...
$\cor_{0,0.5}^n(u,v) - \cor_{0.5,1}^n(u,v)$ would indicate stronger upper tail 
