...
This mechanisms seems likely to apply commonly when co-located species are influenced by the
...
Our results show the distinction between the types of locations relates to the
...
but warm winters do not lead first flights that are any earlier, on average, than those following moderate winters,
producing right-tail dependence between first flights and winter coldness across multiple species;
this common dependence led to right-tail dependence between aphids. Thus winter temperature
fluctuations lead to temporally dispersed early but temporally coordinated late arrival times of aphid
species, a fact that may have pest-control significance. Winter temperature is known to 
...
$x_{s,l}(t)$ for fixed $s$ and $l=1,\ldots,L$. One of the consequences studied by @Ghosh_copula
...
the $x_{s,l}(t)$ ($l=1,\ldots,L$), if these time series are positively associated with each other, with right-tail 
(respectively, left-tail) dependence tending to produce right (respectively, left) skew in the total.
Right skew corresponds to a spatial-total time series with exceptionally large values, 
corresponding to "spiky," unstable dynamics of the total population; left skew 
corresponds to a spatial-total time series with low values, corresponding to dynamics of the total
...
this total has been used in an extensive literature (e.g., REFS) to characterize community stability 
...
Because many communities show synchronous dynamics between some species pairs 
$x_{s_i,l}(t)$ and $x_{s_j,l}(t)$, and compensatory dynamics between other pairs,
next research steps should include the study of tail dependence between compensatory species
within a local community. Our *Ceratium* were synchronous, so this was not possible 
with our population density data. Furthermore, *Ceratium* are only part of the 
phytoplankton community in UK seas. It may be advantageous for future work to use 
data characterizing an entire competitive community. For instance, the data of REF 
