...
\end{center}
\end{figure}
<!--***DAN: Shya, the site number on panel C for site 11 is blue, but I think it should be red
based on the numbers in Fig. S4. Please check. Whatever bug caused 11 to be blue instead of red, 
please don't just fix it for this one case, think about other implications the bug will have 
had, if any, and fix it so those implications are also fixed.
***Shya: I don't think it's a bug, Fig.S4 shows number of left and right tail dep. cells 
whereas site codes are in red or blue color depending on values or fraction of 
those cells (i.e. F_C,L or F_C,R) not number (N_L^n, N_R^n).-->
...
<!--***DAN: Shya, I do not see any reason to use Pearson in Fig. 4 and Kendall in Fig. 5. Why was this done?
If there was a good reason to use Kendall for Fig. 5, then please add text below stating what that 
reason is. I will incorporate the reason into the narrative. If there was no good reason, please change your 
code so Pearson is used for Fig. 5 and let me know when that is done.
***Shya: done.-->
...
to species level ($A_C^n$ or $\alpha_C^n$). 
We did a two-tailed correlation test between $A_C^n$ and $A_D^n$ for all selected location $n$ 
...
10 locations, we computed the Pearson correlation coefficient $(P)$ between
$\alpha_C^n(i)$ and $\alpha_D^n(i)$, where
...
for 8 out of 10 sites (based on p-value with $0.05$ significance level, Fig. \ref{fig_aphid_multipanel}),
...
\end{figure}
<!--***DAN: 
1) Shya, please make the panel labels (A) and (B) appear in the above figure in a similar way
to how they appear in the other figures
2) Please change the axis labels to correspond with the new notation
***Shya: done.
-->
...
\caption{For 8 out of 10 sites, the Pearson correlation ($P$) between $\alpha_C^n(i)$ and $\alpha_D^n(i) ; i = 1, 2, \dots, 20$, 
