...
We show this asymmetry at species level to be due to similar 
...
planktons or aphids. For example, consider a situation when harsh winter delayed first flight of aphids 
...
We will also present two alternative approach for investigating (Q2) for \textit{Ceratium spp.} and aphids. 
...
As dinoflagellates are considered to have an important role in the phytoplankton bloom,
...
first, we chose a subset, from 26 locations, which had at least 36 years of data available for all species.
...
we considered only those selected locations which had at least two 
\textit{Ceratium} sp. along with their predator C. fin. We also considered
time series of sea surface temperature (for growing season) at each selected grid cell (Fig. \ref{SM-fig_plankton_map}) 
to be one of the key drivers
which caused synchronous response among \textit{Ceratium} sp.

As a second spatio-temporal dataset, we used phenological data of aphid first-flight dates for 10 selected 
...
by requiring at least 30 years of data points available for all sp. at each site 
...
temperature (December, $(t-1)^{th}$ year to March, $t^{th}$ year) was considered a 
main driver causing spatially synchronous changes in arrival date of flying aphids at each site.
...
For the $n^{th}$ site, we estimated $C^n_{L-U}(i, j)$ for all pairwise combination of $(i, j)$ and counted
the number of lower ($N_L$) and upper ($N_U$) tail dominant species pair for which 
...
overall tail-dependence. But to get a more precise answer, whether the asymmetric
...
of driver dependencies as $A_D^n = \sum_{i=1}^{s}\sum_{k=1}^{d} C^n_{L-U}(i, k)$ and tested if there was
...
species time-series from a given location.
...
Fig. \ref{fig_CorlmCoru_ff_map_loc2_5}(B) and (C), respectively. 

Major asymmetric tail-dependence 
observed with one set of population data (plankton)
and one set of phenological data (aphids firstflight) answered (Q1), indicating that inter-specific 
synchrony could be described through tail-dependence
...
We tried a similar approach as of \textit{Ceratium} sp. to explain the causal mechanism for 
asymmetric tail-dependence at species level. <!--add correlation and non-significant p value here-->Since aphids were 20 different sp. which 
...
(Fig. \ref{SM-fig_aphid_scatter}). In an alternative approach, for each of the selected 
...
which answered the probable causal mechanism behind inter-species synchrony (Q2). 
...
<!--summary results-->
We found  substantial inter-specific synchrony with asymmetric tail-dependence between different species sharing same location through time.  With two long-term spatio-temporal datasets from two different taxa, we showed partial-Spearman correlation 
...
verified our hypothesis with two alternative approaches for planktons and aphids, respectively. We think 
...
an ecosystem. We expect our resuts to generalize to other plankton systems subject to limiting factors.   In most freshwater lakes algal-productivity is limited by the availability 
...
$N_U > N_L$) at 8 out of 10 sites. This was attributed to the fact that aphid firstflight dates
were limited by cold winter effects (Fig. \ref{fig_intro}D), causing a dependence structure like Fig. \ref{fig_intro}E 
more frequently than Fig. \ref{fig_intro}C. We believe this is explained by the particular sensitivity of overwintering aphids to frost conditions (killing non-winged morphs) delaying the population reaching the winged-morph stage, and first flight.
