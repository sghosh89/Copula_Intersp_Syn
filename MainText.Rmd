...
<!--***DAN: Shya, do you mean BiCopIndTest here? Please check, and change everywhere using a search and replace if needed,
in both the main text and supp mat. Please also search for BiCopIndepTest in the BIVAN paper as well and change there, too, if needed.I could not find a function BiCopIndepTest in VinCopula, but I did find a BiCopIndTest.-->
or if independence was rejected but the Kendall correlation
<!--***Shya : in my code I used Kendall, not Spearman-->
<!--***DAN: Shya, it seems likely it would be very easy for you to change this to Spearman and rerun.
If it would be easy, please do it, but note the changes you make in case we have to revert back.
If it would be hard, don't do it, come and speak to me first, letting me know why it would be hard.
It looks weird to be using Kendall here when we have been using Spearman elsewhere. I think it will 
make very little difference to our results if you change this, but will be awkward to explain if we leave it as is. 
If you do it, let me know the outcome. The reason I suspect it will make no difference is I generated 1000 simulated
datasets and computed kendall and spearman for all of them and looked how often the signs disagreed. It was only 19 times.-->
...
<!--***Shya : in my code I used Kendall, not Spearman-->, 
<!--***DAN: see my above note about changing deom Kendall to Spearman, which applies here, too-->
...
by @Ghosh_copula is a simple and effective measure of tail dependence for ecological applications.
Our results also demonstrate a mechanism of asymmetric tail dependence between species: it can
...
omit phenomena that seem likely to be important at least for two major topics of interest in ecology:
synchronous/compensatory dynamics of species within a community and their influence on 
community stability; and shifting phenologies and the match-mismatch hypothesis. We revisit 
these connections below.

<!--P: 2) Interpretation of whole chain of causal events for aphids-->


<!--P: 3) Same for Ceratium, if possible - see line 612 of main text-->
...
<!--P: 5) our results were sufficient to show tail dependence can be an important factor in commuity dynamics, but we need to consider compensatory dynamics as well, and need to use datasets that actually represent a whole community, e.g., grassland data - future work linking to community dynamics and stability-->
