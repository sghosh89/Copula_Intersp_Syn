...
$C^n$ and $D^n$ (Methods), horizontally concatenated, for example locations $n=2$ (A) and $n=5$ (B).
See Table \ref{tab_plankton_aphid_info} for species names.
...
Green dots in $D^n$
represent variables which were originally negatively associated, so the negative
of winter temperature was used for calculating tail dependence (Methods); this happened in all cases
...
hypothesis from the Introduction for a cause of tail dependence between co-located
...
Our second analysis using aphids, based on the statistics $\alpha_C^n(i)$ and $\alpha_D^n(i)$ (Methods),
provided further evidence supporting our hypothesis for a cause of tail dependence between co-located
species (Introduction). For 8 of 10 sampling locations, $\alpha_C^n(i)$ and $\alpha_D^n(i)$
...
between species for aphid and plankton time series. Panels show $A_C^n$ versus $A_D^n$ (Methods) across
...
an ecosystem. 
***DAN: This is a very general intro sentence. The para is actually about freshwater plankton, so I suggest you replace "ecosystem" with "freshwater plannkton ecosystem" 
We expect our resuts to generalize to other plankton systems subject to limiting factors.
In most 
***DAN: I would say "many" instead of "most" to avoid irrelevant complaints from referees about their opinions on the  relative commonness of P versus N and other limitations in freshwater systems
freshwater lakes algal-productivity is limited by the availability 
...
***DAN: I would say it *may* be so represented - we have not checked. Also, use left-tail, not lower-tail
But excessive fertilizers (through sewage or agricultural runoff) beyond a threshold 
can catalyze plankton-growth. 
***DAN: This seems like it would be right-tail dependence, right? How do you reconcile?
Phytoplankton bloom, an
