...
that rank of the smallest value is 1.-->
...
*partial Spearman correlation*, which we here define following @Ghosh_copula. 
...
but the sum is over only the indices $t$ for which $u_t+v_t > 2l_b$ and 
$u_t+v_t < 2u_b$. The partial Spearman correlation is not defined if there
are no points within the band. The partial Spearman correlations 
$\cor_{0,b}$ and $\cor_{1-b,1}$ for $b\leq 0.5$ 
measure tail dependence, and can be compared
via a difference, $\cor_{0,b}-\cor_{1-b,1}$, to measure asymmetry of tail 
dependence. 
The sum of $\cor_{0,1/2}(u,v)$ and 
$\cor_{1/2,1}(u,v)$ (or the sum of
$\cor_{l_{b_k},u_{b_k}}(u,v)$ for any other choice of bands 
$(l_{b_k},u_{b_k})$ that partition the interval $(0,1)$) equals the standard 
Spearman correlation, as long as no points happen to lie directly 
on the bounds $u+v=2l_{b_k}$ and $u+v=2u_{b_k}$. 

<!--***DAN: got to here 2019 07 02-->
