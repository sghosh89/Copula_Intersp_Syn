...
To estimate lower and upper tail dependence in specific, Eq. \ref{eq.Cor} was 
...
We defined lower and upper tail
asymmetric estimates on species level for $n^{th}$ site as
$A_S^{n+} = \sum_{i=1}^{s}\sum_{j=1}^{s}C^n_{L-U}(i, j)$ where $C^n_{L-U}(i, j) >0$ 
and $A_S^{n-} = \sum_{i=1}^{s}\sum_{j=1}^{s}C^n_{L-U}(i, j)$ where $C^n_{L-U}(i, j) <0$, 
...
species could show synchronous behavior with specific tail-dependent asymmetry. 
