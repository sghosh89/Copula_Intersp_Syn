...
of the relationship between variables;  these approaches are based on the idea of the *copula*. Copulas separate the 
...
Following @Ghosh_copula, we introduce some rank-based ideas
that give a conceptual flavor of copulas before subsequently focusing on a particular 
aspect of copula structure, \emph{tail association}. 
A conceptual flavor of copulas helps to understand tail associations, but our focus 
on tail associations makes it unnecessary to introduce copulas systematically;
and brief [@Genest2007; @Anderson2018; @Ghosh_copula] 
and comprehensive [@nelsen2006_copula; @joe2014_dependence; @MaiScherer2017] introductions 
are available elsewhere.
The isolated information on the association between $x_t$ and $y_t$ is revealed by the plot of $u_t$
...
the *normalized rank plot* for $y_t$ and $x_t$; this plot reflects the copula structure of 
$(x_t, y_t)$ [@Ghosh_copula]. For instance, the normalized rank plots for
...
on association between the variables. It is likewide the purpose of copula approaches
to separate association information from information on marginals. @sklar1959_theorem
proved a seminal theorem stating that copulas give a unique and complete description
