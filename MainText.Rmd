...
from mutual tail dependence of these quantities with an environmental driver. 
We argue that our understanding of commuity dynamics and stability, and of
interacting phenologies, can be substantially improved in future work by taking into
...
<!--***DAN: Shya, I commented the below paragraph you wrote because it needs references if
it is to be included. There are some good reflections in the para, but there are a lot of 
statements made that need to be substantiated,
and one way to do that is adding references. I encourage you to work on this paragraph
to substantiate the statements made, by adding references and perhaps in other ways, and
once it is improved in that way we can add it back in by uncommenting it. I have also
put in some comments for your consideration.-->
<!--***Shya: I edited the following para, please check if it works or not-->
Monitoring tail dependence (a measure of "synchrony") could reveal interesting information about 
freshwater plankton ecosystem. We expect our resuts to generalize to other plankton systems
subject to limiting factors. In many freshwater lakes algal-productivity is limited by the availability 
of inorganic phosphorus [@schindler2006recent], which may reflect left tail dependence (stronger association
...
can catalyze plankton-growth. It is interesting to study, whether this  excess P level 
could alter the tail dependence from left to right tail significantly. However, if the lake water is 
first well fertilized with phosphate, its productivity could then respond 
to nitrate addition, indicating that this source of inorganic nitrogen 
could be the secondary limiting factor [@elser1990phosphorus].
Phytoplankton bloom, an ecosystem-level perturbation, occurs when compensatory dynamics 
between perturbation-sensitive phytoplankton and perturbation-tolerant taxa
become weak and so dramatic changes occur in the total
biomass on aggregate level. As 
phytoplankton communities are diverse in response to change
...
even could be used as an "early warning signal" before the actual bloom occurs. Skewness based measure
was already used as an early warning signal in different ecosystem including 
lakes [@guttal2008changing; @carpenter2011early; @butitta2017spatial] and as tail-dependence 
is related to skewness (discussed later), we think it is worth to study in future how monitoring tail dependence 
can predict for any regime shift or spatial bloom as an early warning signal.
...
But @Ghosh_copulas did not consider co-located species, a context important for
