...
(Q1) Do different species show a synchrony with a specific (lower- or upper-) 
tail-dependence?
and (Q2) if they do show such asymmetric tail-dependence (this is a property of 
non-Normal copula structure) then
...
We used phenological data of aphid first-flight dates from 11 locations (Fig. \ref{SM-fig_aphid_map}) 
...
Survey suction-trap dataset [@harrington2014; @bell2015]. Data pre-processing for the aphids
was the same as that of [@sheppard2016]. 

Our second spatio-temporal dataset comprised average annual 
...
of [@sheppard2017]. Sheppard et al. [@sheppard2017] converted monthly 
values to yearly values by averaging over all months. But here we only allowed maximum
4 months in a year with missing data which were replaced by median value for 
that month (drawn from all years in which data was available). As dinoflagellate 
are considered having important role in phytoplankton bloom,
we screened 4 Ceratium species (out of 9 phytoplanktons) and 
their generalist predator zooplankton \textit{Calanus finmarchicus} from 15 
selected locations. Locations were selected based on presence of atleast any two 
ceratium species along with their predator having minimum 36 years of data. 
...
\caption{Lower tail - upper tail dependence matrix ($C_{L-U}^n$ for location index n = 6 and n = 7) with first flight
...
negative cells for 20$\times$ 20 matrix; $N_L > N_U$ in $(A)$ (respectively, $N_U > N_L$ in $(B)$)
indicates major lower (respectively, upper) tail dependence in sp-sp interaction matrix. 
Yellow cells indicate independent relationship. $(C)$ Proportion of tail dominant 
contribution ($f_L$ for lower tail and $f_U$ for upper tail) were plotted for each 
location separately for 20 $\times$ 20 sp-sp interaction matrix.\label{fig_CorlmCoru_ff_loc67map}}
\end{center}
\end{figure}
