...
Recent study [@Ghosh_copula] showed potential use of copula in detecting
"spatial synchrony" between metapopulations. 
We used this copula concept to estimate synchronous and/ or compensatory
response between different species which shared 
same environments. We applied partial-Spearman correlation based approach
...
two associated species' time-series from same 
location lead to asymmetry in inter-specific response. 
With two long-term spatio-temporal datasets (\textit{Ceratium} sp. abundance data and 
aphids firstflight data), we showed overall asymmetric tail-dependence is a common signature for 
inter-specific synchrony within a community, when we considered all combinations of species-pair.
We explained this asymmetry across species level to be in effect due to similar 
asymmetric tail-dependence of the species in response to their biotic/ abiotic drivers.
We considered abundance of generalist predator 
\textit{Calanus finmarchicus} and growing season temperature as two key drivers 
which shaped the tail-dependence between phytoplankton community, whereas dependence in 
aphids firstflight dates were influenced by average winter temperature. With two alternate 
approach for \textit{Ceratium}s and aphids, we found 
significant correlation between asymmetric tail-dependence of species-drivers 
and species-species copula, which explained the probable causal mechanism 
...
causes and consequences of their synchronous response. Studying spatial patterns of 
...
interest to ecologists [@lande1999; @liebhold2004; @walter2017]. But in case of inter-species 
...
In either case, those variables give rise to asymmetric copula structure. Several
families of copula are available with positive tail dependence exclusively in either
or both tail [@joe2014_dependence; @nelsen2006_copula], in addition to 
a copula from 'Normal' family, having zero tail-dependence in 
both tail, and hence considered as a symmetric copula.
Following we will discuss the relevance of 
...
Asymmetric tail-dependence for the overall community is expected to observe when most of its 
member species go synchronously rare or synchronously abundant. Net productivity for 
such ecosystem could be constrained by some limiting factors (Leibig's law)
which included abiotic (temperature, windspeed, salinity 
or moisture) and biotic (predators, food-webs) interactions.
For example, extreme temperature had profound effect on plant-productivity 
or its pollination rate depending upon the species [@hatfield2015]. Copula structure with 
lower (or upper) tail-dependence could reveal the scenario whether any abiotic or biotic variable
causes such asymmetric tail dependences in inter-species dependence structures. We will
ellaborate this idea in the following example.

<!--earlier work on sp-driver relationships : aphids and planktons-->
Earlier work showed winter temperature had profound effect on spatial 
synchrony between aphids [@sheppard2016]. Similarly, plankton 
community in the North Sea was found to be affected by 
climatic variables like temperature and 
its predator zooplankton [@defriez2016climate; @sheppard2017; @sheppard2019].
Inspiring from those findings, in this paper, we hypothesize possible biotic and/ or 
abiotic drivers causes asymmetric tail-dependence in inter-specific synchrony, if any found for 
planktons or aphids. For example, consider a situation when harsh winter delayed arrival of aphids 
on a site, i. e., aphids firstflight date was negatively correlated with winter temperature. If we 
made a copula with aphid firstflight dates and negative winter temperature, then it could be 
either lower tail (Fig. \ref{fig_intro}B) or upper-tail dependent (Fig. \ref{fig_intro}D) depending on 
individual aphid's response. Fig. \ref{fig_intro}B represents that species $i$ came early when 
the site was warmer than usual (species firstflight date facilitated by warmer temperature), whereas 
Fig. \ref{fig_intro}D represents arrival of species $i$ at a site was limited by cold winter. 
If majority of other aphids also showed similar trend with the temperature 
at a given site as depicted in Fig. \ref{fig_intro}B or D, then we hypothesize, we
would see the corresponding asymmetric tail-dependence between two species ($i \neq j$) firstflight dates 
as depicted in Fig. \ref{fig_intro}C or E, respectively.

In this paper, we will introduce "partial-Spearman correlation" based approach to measure 
...
two different taxa (planktons and aphids) to answer the following questions: 
...
We will also present two alternative approach for investigating (Q2) for planktons and aphids. 
...
for the points lying on left of solid black or red lines) than in upper tail (considered for the points lying 
on the right of solid black or blue lines). Asymmetric tail dependence in species-driver (negative 
temperature was considered as a driver for aphid species firstflight date) copulas (B, D) caused similar 
...
Data preprocessing steps are same as used in [@Ghosh_copula]. 
...
(e.g. see Fig. \ref{fig_intro}A) of species pair $(i, j)$ taken from $n^{th}$ site and
$p$ equispaced patitions were made to the diagonal (dotted line in Fig. \ref{fig_intro}A) 
of the box. To estimate lower and upper tail dependence, Eq. \ref{eq.Cor} was 
applied for the points lying in the first and the last triangular partitions, respectively. 
Given $0 \leq b (= 1/p) \leq 0.5$, we used the 
shorthand notation for lower tail dependence as $C^n_L(i, j) = \cor_{l_b,u_b}^n(u,v)$ with $(l_b,u_b)=(0,b)$
and for the upper tail dependence as $C^n_U(i, j) = \cor_{l_b,u_b}^n(u,v)$ with $(l_b,u_b)=(1-b,1)$,
respectively. We considered three equispaced partitions ($b = 1/3$) for plankton data; $C^n_L(i, j)$
...
points of aphid firstflight data, we examined two equal halves ($b = 1/2$) of the unit 
square. $C^n_L(i, j)$ and $C^n_U(i, j)$ were measured for the points scattered on the left and right side of 
...
in inter-species response, as we will see with aphid (20 species) firstflight data. 
...
$\alpha_D^n(i) = \sum_{k=1}^{d} C^n_{L-U}(i, k)$, respectively, for a given $i^{th}$ species. Significant correlation 
...
tail dependence matrix ($C_{L-U}^n$) for site $n=12$ (Fig. \ref{fig_CorlmCoru_plankton_map_loc12_26}B) and $n=26$ (Fig. \ref{fig_CorlmCoru_plankton_map_loc12_26}C) were shown as two examplary cases (for 
...
stronger upper tail dependence ($N_L = 2, N_U = 4$). We got similar findings for aphid firsflight data (Fig. \ref{fig_CorlmCoru_ff_map_loc2_5}). Proportion of lower 
...
dependence matrix $C_{L-U}^n$ were pictured in 
Fig. \ref{fig_CorlmCoru_ff_map_loc2_5}(B) and (C), respectively. Major asymmetric tail-dependence 
observed with one population (planktons)
and one phenological (aphids firstflight) data answered (Q1) that inter-specific 
synchrony was captured through tail-dependence
...
\caption{$(A)$ Proportion of tail dominant contribution ($f_L$ for lower tail and $f_U$ for upper tail 
showing on a color scale between 0 to 1) in overall asymmetry for each 
selected 10 locations (see Fig. \ref{SM-fig_aphid_map}) plotted on map summarizes the overall 
...
($C_{L-U}^n$) with first flight data of 20 aphid species have shown in (B) for location n = 2 and
...
\textit{Ceratium} sp. ($s = 4$), we considered abundance of predator C. fin 
and growing season temperature as two key drivers ($d = 2$) for inter-specific synchrony. 
We observed species synchronous response towards biotic and/ or abiotic stress
...
(Fig. \ref{fig_plankton_scatter}). For aphids, we considered winter average temperature as 
the main driver $(d = 1)$ for their synchronous firstflight date at a given location, $n$. 
We tried similar approach as of \textit{Ceratium} sp. to explain causal mechanism for 
asymmetric tail-dependence on species level. Since, aphids were 20 different sp. which 
varied widely in their inter-specific response and were also expected 
not having direct interactions as of \textit{Ceratium}s, it is obvious that
$A_S^n$ and $A_D^n$ on total asymmetry level could not illuminate any causal relationship 
(Fig. \ref{SM-fig_aphid_scatter}). On an alternative approach, for each of the selected 
10 locations, we computed Kendall correlation coefficient $(K)$ between
...
which answered the probable causal mechanism behind inter-species synchrony as addressed in (Q2). 
...
\textit{Ceratium} sp., numbers in the plot represent the side code $n$.\label{fig_plankton_scatter}}
\end{center}
\end{figure}
...
average temperature is the reason for similar tail-dependence between $i^{th}$ species
to other species.\label{fig_aphid_multipanel}}
\end{center}
\end{figure}
...
<!--summary results-->
We found asymmetric tail-dependence between different species sharing same location through time, which 
indicated there was substantial inter-specific synchrony between them. With two long-term 
spatio-temporal datasets from two different taxa, We showed partial-Spearman correlation 
estimates at extreme tails of copulas could be used as a simple tool to measure dependence 
...
verified our hypothesis with two alternative approach for planktons and aphids, respectively. We think 
our approach could be useful in investigating inter-species dependence structures for more
complex systems like foodwebs with multiple trophic interactions.

<!--what extra info do you get by monitoring tail dep. for planktons?-->

Monitoring tail dependence (a measure of "synchrony") could reveal interesting information about 
an ecosystem. In most freshwater lakes algal-productivity is limited by the availability 
...
phytoplankton and perturbation-tolerant taxa become weak and thereby affecting the total
biomass on aggregate level. As phytoplankton communities are diverse in response to change
in the nutrient level in a lake, monitoring their individual dynamics through copula 
...
$N_U > N_L$) at 8 out of 10 sites. This was attributed to the fact that aphids firstflight dates
were limited by too cold winter temperature (Fig. \ref{fig_intro}D), causing Fig. \ref{fig_intro}E 
more frequent than Fig. \ref{fig_intro}C. 
