...
($C_{L-U}^n$) with first flight data of 20 aphid species have shown in (B) for location n = 6 and
in (C) for location n = 7, for other locations see Fig. \ref{SM-fig_CorlmCoru_ff_all_loc}. 
...
except $n = 3$ (as site 3 was independent). We got significant 
positive Pearson correlation ($0.699$)<!--caution : not automated link--> with p-value $<<0.05$, 
which confirmed our hypothesis for (Q2) that tail-asymmetry translated from driver to species level 
(Fig. \ref{fig_plankton_scatter}).

<!--explain aphids scatter plot failed and how aphids motivates the species specific approach-->

For aphids, we considered winter average temperature as the main driver $(d = 1)$ for their
synchronous firstflight date at a given location, $n$. For each of the selected 10 locations,
we computed Kendall correlation coefficient $(K)$ between $\alpha_S^n(i)$ and $\alpha_D^n(i)$, where
...
\textit{Ceratium} sp., numbers in the plot represents the side code $n$.\label{fig_plankton_scatter}}
\end{center}
\end{figure}
