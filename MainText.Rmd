...
We here study the nature and causes of tail associations
between phenological or population-density variables of co-located species. 
We employ a simple method of measuring tail association called the *partial Spearman correlation*.
...
also commonly show asymmetric tail association. We provide evidence that tail association
between two or more species population density or phenology time series can be inherited
from mutual tail association of these quantities on an environmental driver. 
...
account the asymmetry of tail associations. 

\noindent \textbf{\textit{Keywords:}} tail association, copula, inter-species synchrony, aphids, plankton
...
studies which used a normal copulas [@Valpine2014; @Anderson2018; @Popovic2019], 
...
*tail dependence* - the associations between the more extreme 
values of distributions (Fig. \ref{pedagogfig}). For two 
...
other [@Ma2017]. If synchronous fluctuations show right-tail association,
...
dependence in their tail-associations, with a focus on positively associated variables
...
of different co-located species commonly show asymmetric tail associations? (Q2) If so, 
what are the causes of these patterns? We regard our investigation
as a first step toward a better understanding of the potential importance of copula
structure and asymmetric tail association for such central ecological topics as synchrony and 
...
\caption{Pedagogical figure for introducing copulas, asymmetric tail association, and partial Spearman correlation.
(A, B) Two pairs of variables that have identical Pearson (P) and Spearman (S) correlation coefficients
but differ markedly in the nature of the association, panel A showing stronger dependence in 
left- than right-tail association and panel B showing the reverse. (C, D) Copula 
...
asymmetry of tail association between aphid or *Ceratium* time series at $n$. Denote by
...
tail association (or anything else about dependence) for independent
...
We also computed a matrix $D^n$ which quantifies tail association between 
...
for which association was stronger in left- rather than in right-tail.
...
general measure of asymmetry of tail association in location $n$. And we calculated
...
stronger right-tail (respectively, left-tail) association 
with each other at that same location. 
...
$i$, we computed the sum $\alpha^n_C(i)$ of all non-NA entries $C^n(i,j)$, 
<!--***Shya: Suppmat line 815, I actually used mean, not sum-->
...
negative ones for greater right-tail association. We then defined
...
association dominated. Associations between aphid time series were always positive 
