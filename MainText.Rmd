...
<!--***DAN: Dan, make sure this is explicitly mentioned again in the Results-->
Tail dependence
...
Recall that an environmental covariates was reversed (its negative was used)
...
because we only had data for at most $4$ *Ceratium* species per sampling
...
matrix ($C^n$ for location n = 12 and n = 26) with Ceratium abundances 
...
$N_L^n$ and $N_R^n$ are the count of cells with positive and 
negative values for positively correlated species pair. $N_L^n > N_R^n$ in $(B)$ 
indicates major lower tail dependence in sp-sp interaction matrix. Similarly, $N_R^n > N_L^n$ in $(C)$ indicates more upper
