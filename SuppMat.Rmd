...
author: "Shyamolina Ghosh, Lawrence W. Sheppard, other authors, Daniel Reuman"
...
library(kableExtra)
library(dplyr)
aphid_info <- tibble(c0=c(1:20),
                     c1=c("Apple grass aphid","Bird cherry oat aphid","Black bean aphid","Blackberry cereal aphid","Blackcurrant sowthistle aphid",
...
knitr::kable(aphid_info, "latex", booktabs = T, linesep = "",align="c",
      caption = "Species names of 20 aphids for which data were available \\label{tab_aphid_info}",
      col.names = NULL)%>%
     column_spec(column = 3,italic=T)%>%add_header_above(header=c("Species id" = 1,"Common name" = 1, "Latin binomial" = 1))
```

```{r tab_plankton_info, echo=F, results='asis',message=F}
library(tibble)
library(kableExtra)
plankton_info <- tibble(c1=c(1:22),
                        c2=c('Decapoda larvae','Echinoderm larvae',
                        'Ceratium fusus','Ceratium furca','Ceratium tripos','Ceratium macroceros',
                          'Centropages typicus','Oithona sp.', 'Acartia sp.',
                          'Calanus finmarchicus',
                          'Calanus stages I-IV','Para-Pseudocalanus spp.','Temora longicornis',
                          'Calanus helgolandicus','Metridia lucens', 'Pseudocalanus sp.','Euphausiids',
                     'Thalassiosira sp.','Rhizosolenia styliformis','Proboscia alata',
                     'Pseudo-nitzschia delicatissima','Pseudo-nitzschia seriata')
)

cns<-c('Echinoderm larvae','Euphausiids','Decapoda larvae')
'%ni%' <- Negate('%in%')
idn<-which(plankton_info$c2%ni%cns)
knitr::kable(plankton_info, "latex", booktabs = T, linesep = "",align="c",
      caption = "Species names of 22 plankton taxa for which data were available \\label{tab_plankton_info}",
      col.names = NULL)%>%
      row_spec(c(3:6,10), bold = T, color = "blue")%>%row_spec(row=idn,italic=T)%>%add_header_above(c("species id"=1,"Plankton taxon name"=1))
```
...
ff_selected_loc<-c()
for(loc in c(1:11)){
  temp<-good_splist(d_allsp=data_ff_with_temp,loc=loc,data_pt_thrs=29) # at least 29 datapts for each pairwise species taken
...
saveRDS(ff_selected_loc,"./Results/aphid_results/ff_selected_loc_data_pt_thrs_29.RDS")
...
numloc<-length(ff_selected_loc)
stat_ff_ln_all<-vector("list",numloc)
names(stat_ff_ln_all)<-paste("loc",ff_selected_loc,sep="")

for(loc in c(1:numloc)){
  nm<-names(stat_ff_ln_all)[loc]
  if (!dir.exists(paste(resloc,nm,sep=""))){
  dir.create(paste(resloc,nm,sep=""))
  }
  
  good_sp <- good_splist(d_allsp=data_ff_with_temp,loc=ff_selected_loc[loc],data_pt_thrs=29)
  
  #cat(paste("start-time: ",Sys.time(),"\n"))
  resloc2<-paste(resloc,names(stat_ff_ln_all)[loc],"/",sep="")
  
  stat_ff<-multcall(d_allsp=data_ff_with_temp,
                          loc=ff_selected_loc[loc],
                          pfname=paste(resloc2,"ff",sep=''),
                          good_sp=good_sp,
                          nbin=2,npa_stats = "cor")
  
  stat_ff_ln_all[[loc]]<-stat_ff
  
  #cat(paste("stop-time: ",Sys.time(),"\n"))
}

saveRDS(stat_ff_ln_all,paste(resloc,file="stat_ff_ln_all.RDS",sep=''))
```

```{r plot_res_css_ff_npa, echo=F, results="hide",message=F,cache=T, cache.extra=list(seed,stat_ff_ln_all,mtime("NonParamStat_matrixplot.R"),mtime("mycorrplot.R"))}

set.seed(seed)
source("NonParamStat_matrixplot.R")

resloc<-"./Results/aphid_results/ff_npa_stat_results/"
npa_diff_ff_ln_all<-NonParamStat_matrixplot(data_ln_all = stat_ff_ln_all,
                        resloc=resloc,
                        nvar=1,nvar_names=c("Temp."),tagon=T,npa_stats="cor")
saveRDS(npa_diff_ff_ln_all,paste(resloc,file="npa_diff_ff_ln_all.RDS",sep=''))
```
...
summary_LU_ff_sp_only<-npa_diff_ff_ln_all$summary_LU_CorlmCoru

# on map
pdf("./Results/aphid_results/Corstat_LmU_values_on_map_sp_only.pdf",height=9,width=9)
...
legend("topright", legend=c(expression(f[L]),0.2,0.4,0.6,0.8,1),cex=2,
...
legend("topleft", legend=c(expression(f[U]),0.2,0.4,0.6,0.8,1), cex=2,
       ...
       pt.cex=2,horiz=F,bty="o")
mtext("Tail-dep for sp-sp 20x20 matrix",cex=1.5)
...
x<-npa_diff_ff_ln_all$CorlmCoru_all_ln_list
...
pdf("./Results/aphid_results/Corstat_scatter_LmU_values.pdf",height=8,width=8)
op<-par(mar=c(5.1, 5.1, 4.1, 2.1), mgp=c(3, 1, 0))
plot(xx,yy,xlab="(L+U) for sp-temp.",ylab="(L+U) for sp-sp matrix",col=rgb(0,1,0,0.4),pch=19,cex=2,cex.lab=2,cex.axis=2)
...
LTmUTdep_matlist_ff<-npa_diff_ff_ln_all$CorlmCoru_all_ln_list
pdf("./Results/aphid_results/Corstat_LmU_avg_sp_temp_multipanel_plot.pdf",height = 35,width = 45)
op<-par(mfrow=c(4,3),mar=c(15,10,10,5.5), mgp=c(3,4,0))
...
for(loc in 1:length(LTmUTdep_matlist_ff)){
  ...
  t<-t[,1]/t[,2]
  t<-sum(t>0,na.rm=T)
  plot(c(1:20),avg_s,type="b",col="red",xlab="",cex=4,pch=16,ylim = ylim,ylab="",cex.axis=7,xaxt="n")
  axis(side = 1, at = seq(from=1,to=20,by=1), cex.axis=8)
  mtext("species", side=1, line=11, col="black", cex=7)
  ...
  mtext(paste("Loc =",loc,sep=""),cex=7,side=3,line=-7,adj=0.05)
...
legend("bottomright", c("row-wise average of \nsp-sp interaction matrix \n(lower - upper tail dep.)\n", "sp-temperature \ninteraction cell \n(lower - upper tail dep.)"), col = c("red","blue"),bty="n",
...
# Now, include growing season temp. data for planktons as 11 -th species
  ...
  for(loc in 1:length(data_plankton)){
    ...
    for(i in 1:nsp){
      for(j in 1:nsp){
        ...
        ms<-vivj_matrix(d_allsp = data_plankton,loc=loc,i=i,j=j,ploton=T,tagon=F)
...
call_summary_each_loc_plankton<-function(rawdata,ploton){
  ...
  summary_each_loc<-data.frame(sp=c(1:10),n=NA,nT=NA,nrep0=NA,avg.Abun.=NA)
  ...
  for(sp in c(1:5)){
    ...
    if(ploton==T){
      ...
      mtext(paste0("sp=",sp,
                 ...
                 ", nrep0=",summary_each_loc$nrep0[sp],
...
loc_ts_n<-which(locwise_n$n>35)  # location based on 1st threshold : check on n

min_n<-min(locwise_n$n[loc_ts_n]) # minimum number of data points among all locations after 1st threshold

thrs<-0.1 # percentage allowed for ties : 2nd threshold
...
for(i in 1:length(loc_ts_n)){
  splist<-c(1:5)
  loc<-loc_ts_n[i]
  badsp<-which(summary_all_loc_raw_plankton[[loc]]$nrep0>thrs*min_n)
...
```{r plankton_npa, echo=F, cache=T, cache.extra=list(seed,data_plankton,mtime("good_splist.R"),mtime("NonParamStat.R"),mtime("vivj_matrix.R"),mtime("CopulaFunctions.R"), mtime("CopulaFunctions_flexible.R"))}
...
stat_plankton_ln_all<-vector("list",numloc)
names(stat_plankton_ln_all)<-paste("loc",good_loc,sep="")

for(loc in c(1:numloc)){
  nm<-names(stat_plankton_ln_all)[loc]
  if (!dir.exists(paste(resloc,nm,sep=""))){
  dir.create(paste(resloc,nm,sep=""))
  }
  ...
  #cat(paste("start-time: ",Sys.time(),"\n"))
  resloc2<-paste(resloc,names(stat_plankton_ln_all)[loc],"/",sep="")
  
  stat_plankton<-multcall(d_allsp=data_plankton,
                          loc=good_loc[loc],
                          pfname=paste(resloc2,"plankton",sep=''),
                          good_sp=good_sp,
                          nbin=2,npa_stats = "cor")
  
  stat_plankton_ln_all[[loc]]<-stat_plankton
  
  #cat(paste("stop-time: ",Sys.time(),"\n"))
}

saveRDS(stat_plankton_ln_all,paste(resloc,file="stat_plankton_ln_all.RDS",sep=''))
```

```{r plot_res_plankton_npa, echo=F, results="hide",message=F,cache=T,warning=F, cache.extra=list(seed,stat_plankton_ln_all,mtime("NonParamStat_matrixplot.R"),mtime("mycorrplot.R"))}
set.seed(seed)
source("NonParamStat_matrixplot.R")
resloc<-"./Results/plankton_results/npa_stat_results/"
npa_diff_plankton_ln_all<-NonParamStat_matrixplot(data_ln_all = stat_plankton_ln_all,
                        resloc=resloc,
                        nvar=2,nvar_names=c("CALFIN","Temp."),tagon=T,npa_stats = "cor")
saveRDS(npa_diff_plankton_ln_all,paste(resloc,file="npa_diff_plankton_ln_all.RDS",sep=''))
```
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=8 cm,height=8 cm]{./Results/aphid_results/map_plot_css_ff_selected_loc.pdf} 
\caption{Position of 11 suction traps that sample flying aphids. We considered each location to collect 35 year long time series (1976-2010) data for 20 chosen aphid species firstflight date. \label{fig_aphid_map}}
\end{center}
\end{figure}



<!--aphid : npa : corl - coru all location plot-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=6 cm]{./Results/aphid_results/ff_npa_stat_results/loc1/loc1_Corl-Coru.pdf} 
...
\caption{Analogous figure as of Fig. \ref{fig_LTmUT_ff_all_loc} for (lower tail - upper tail) dependence matrix calculated using
$\cor_l - \cor_u$ non-parametric statistic, details were discussed in Appendix \ref{nonparam}. \label{fig_CorlmCoru_ff_all_loc}}
\end{center}
\end{figure}
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=8 cm,height=8 cm]{./Results/plankton_results/map_plot_plankton_selected_loc.pdf} 
\caption{Plankton abundance data were obtained from 26 locations (2$^\circ$ $\times$ 2$^\circ$ grid) [Plankton Recorder (CPR) data set maintained by the Sir Alistair Hardy Foundation for Ocean Science (SAHFOS)] for 56 years (1958-2013). We considered selectively 15 locations (indicated by blue-boxes) based on (i) locations had at least 36 year of data points available for any two ceratium species and zooplankton (CALFIN), (ii) species at considered locations should not be rare (0 count value) for more than 3 years.\label{fig_plankton_map}}
\end{center}
\end{figure}

<!--ceratium : corl-coru plot for all location-->
\begin{figure}[!h]
\begin{center}
\hspace{-2 cm}
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc2/loc2_Corl-Coru.pdf} 
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc3/loc3_Corl-Coru.pdf} 
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc7/loc7_Corl-Coru.pdf}
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc10/loc10_Corl-Coru.pdf}\\
\hspace{-2 cm}
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc11/loc11_Corl-Coru.pdf} 
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc12/loc12_Corl-Coru.pdf} 
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc14/loc14_Corl-Coru.pdf} 
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc17/loc17_Corl-Coru.pdf}\\
\hspace{-2 cm}
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc18/loc18_Corl-Coru.pdf} 
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc20/loc20_Corl-Coru.pdf} 
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc21/loc21_Corl-Coru.pdf}
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc23/loc23_Corl-Coru.pdf}\\
\hspace{-2 cm}
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc24/loc24_Corl-Coru.pdf}
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc25/loc25_Corl-Coru.pdf}
\includegraphics[width=5 cm]{./Results/plankton_results/npa_stat_results/loc26/loc26_Corl-Coru.pdf}
\caption{Analogous figure as of Fig. \ref{fig_LTmUT_ceratium_all_loc} for (lower tail - upper tail) dependence matrix calculated using
$\cor_l - \cor_u$ non-parametric statistic, details were discussed in Appendix \ref{nonparam}.\label{fig_CorlmCoru_ceratium_all_loc}}
\end{center}
\end{figure}
