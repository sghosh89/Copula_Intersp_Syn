...
plot(xxyy$xx,xxyy$yy,xlab=expression(A[D]^n),ylab=expression(A[C]^n),col=rgb(0,0,0,0.2),pch=19,cex=2,cex.lab=3,cex.axis=2)
...
op<-par(mar=c(7,8, 4.1, 2), mgp=c(4, 1, 0))
plot(xx,yy,xlab=expression(A[D]^n),ylab=expression(A[C]^n),col=rgb(0,0,0,0.2),pch=19,cex=2,cex.lab=3,cex.axis=2,ylim=c(min(yy),max(yy)+1))
...
legend("center", c("X = C", "X = D"), col = c("black","grey"),bty="n",
       cex = 9, pch = c(16, 17), lty=c(5,1), xpd = TRUE, horiz = F, inset = c(0.06,0.05)) 
par(op)
dev.off()
```
