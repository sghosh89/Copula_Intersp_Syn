---
title: "Supporting material : A new approach to interspecific synchrony and its causes in population ecology using ``copulas''"
  ...
  for(loc in 1:length(data_plankton)){
    ...
    for(i in 1:nsp){
      for(j in 1:nsp){
        ...
        ms<-vivj_matrix(d_allsp = data_plankton,loc=loc,i=i,j=j,level=0.05,ploton=T,tagon=F,onbounds=T,lb=1/3,ub=2/3)
...
plot(xxyy$xx,xxyy$yy,xlab=expression(A[D]^n),ylab=expression(A[S]^n),col=rgb(0,0,0,0.2),pch=19,cex=2,cex.lab=3,cex.axis=2)
...
<!--skewness : ceratiums-->
```{r skew_ceratium,echo=F,results="hide",cache=T,cache.extra=list(seed,data_plankton,splist_updated,good_loc_plankton,mtime("ncsurrog.R"),mtime("skewness_testing.R"))}
set.seed(seed)
source("skewness_testing.R")
#------Skewness results calculation with 4 ceratium sp. for all selected loc ---------------------------------------------------

good_loc<-good_loc_plankton
pdf("./Results/plankton_results/ceratium_skewness.pdf",height=16,width=16)
op<-par(mfrow=c(4,4),mar=c(4.2,5,1,1),mgp=c(3,1,0))

tab_ceratium_skew_screened_sp<-matrix(NA,nrow = length(good_loc),ncol = 2)
tab_ceratium_skew_screened_sp[,1]<-good_loc
colnames(tab_ceratium_skew_screened_sp)<-c("loc","p_left") 


for(i in 1:length(good_loc)){
  
  loc<-good_loc[i]
  cat("----i = ",i,"----loc = ",loc,"-------","\n")
  ceratium_data<-data_plankton[[loc]][c(1:4)]
  df<-cbind(ceratium_data$Ceratium_fusus$Dat,
            ceratium_data$Ceratium_furca$Dat,
            ceratium_data$Ceratium_tripos$Dat,
            ceratium_data$Ceratium_macroceros$Dat)
  colnames(df)<-names(ceratium_data)
  
  df<-na.omit(df)
  
  temp<-skewness_testing(ts_matrix=df,splist=c(1:ncol(df)),numsurrog=10000,ploton=T,plotcheckon=F,corpres="spearman",resloc=NA)
  #mtext(paste0("loc= ",good_loc[i],", p_left= ",temp[1],sep=""),side = 3, line=-2, adj=0)
  tab_ceratium_skew_screened_sp[i,2]<-temp$p_left
  
}
par(op)
dev.off()
saveRDS(tab_ceratium_skew_screened_sp,"./Results/plankton_results/tab_ceratium_skew.RDS")
```
    ...
    for(i in 1:nsp){
      for(j in 1:nsp){
        ...
        ms<-vivj_matrix(d_allsp = data_ff_with_temp,loc=ff_selected_loc[loc],
                        i=i,j=j,level=0.05,ploton=T,tagon=F,onbounds=T,lb=0,ub=0.5)
      }
    }
}
par(op)
...
op<-par(mar=c(7,8, 4.1, 2), mgp=c(4, 1, 0))
plot(xx,yy,xlab=expression(A[D]^n),ylab=expression(A[S]^n),col=rgb(0,0,0,0.2),pch=19,cex=2,cex.lab=3,cex.axis=2,ylim=c(min(yy),max(yy)+1))
text(xx,yy,ff_selected_loc,pos=3,col="black",cex=1.2)
lines(c(-100,100),c(0,0),type='l',lty='dashed',col="grey")
lines(c(0,0),c(-100,100),type='l',lty='dashed',col="grey")
...
for(loc in 1:length(LTmUTdep_matlist_ff)){
  ...
  plot(c(1:20),avg_s,type="b",col="red",cex=4,pch=16,ylim = ylim, 
       ylab=expression(alpha[X]^n*(i)),cex.lab=12,cex.axis=8,xlab="",xaxt="n")
  ...
  mtext(paste("K =",round(tau,2),", p=",formatC(pval,3,format="f")),cex=6,side=3, line=1.5) #pval rounded upto 3digits to show
...
legend("center", c("X = S", "X = D"), col = c("red","blue"),bty="n",
       cex = 9, pch = c(16, 16), lty=c(1,1), xpd = TRUE, horiz = F, inset = c(0.06,0.05)) 
par(op)
dev.off()
```
...
\caption[Sampling sites on map for Plankton abundance data]{Plankton abundance data were obtained from 26 locations (2$^\circ$ $\times$ 2$^\circ$ grid) [Plankton Recorder (CPR) data set maintained by the Sir Alistair Hardy Foundation for Ocean Science (SAHFOS)] for 56 years (1958-2013). We considered selectively 15 locations (indicated by blue-boxes) based on criterion listed in \nameref{MT-Data}\label{fig_plankton_map}}
\end{center}
\end{figure}
...
\caption[Position of 11 suction traps on map that sample flying aphids]{Position of 
11 suction traps that sample flying aphids. We considered only 10 locations 
(shown within blue boxes) which had atleast 30 years of first flight data for 
20 aphid species during (1976-2010) timespan. \label{fig_aphid_map}}
\end{center}
\end{figure}
...
\caption[Planktons: $C_{L-U}^n$ matrix plot for selected locations]{Analogous figure as of 
\ref{MT-fig_CorlmCoru_plankton_map_loc12_26}(B) and (C) showing 
...
\caption[Aphids: $C_{L-U}^n$ matrix plot for selected locations]{Analogous 
figure as of Figs. \ref{MT-fig_CorlmCoru_ff_map_loc2_5}(B) and (C) showing $C_{L-U}^n$ 
...
<!--aphid scatter plot : Q2 : failed result-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/aphid_results/ff_npa_stat_results/Corstat_scatter_LmU_values.pdf} 
\caption{Aphid first flight data showed no significant relationship between total asymmetry on species level $A_S^n$ and 
on driver level $A_D^n$. Numbers in the plot represent the side code $n$.\label{fig_aphid_scatter}}
\end{center}
\end{figure}
