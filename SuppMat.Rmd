...
call_summary_each_loc_plankton<-function(rawdata,ploton){
  ...
  summary_each_loc<-data.frame(sp=c(1:5),n=NA,nT=NA,nrep0=NA,avg.Abun.=NA)
...
```{r plankton_npa, echo=F, cache=T, cache.extra=list(seed,data_plankton,mtime("NonParamStat.R"),mtime("vivj_matrix.R"),mtime("CopulaFunctions.R"), mtime("CopulaFunctions_flexible.R"))}
...
summary_LU_plankton_sp_only<-npa_diff_plankton_ln_all$summary_LU_CorlmCoru
...
x<-npa_diff_plankton_ln_all$CorlmCoru_all_ln_list
for(loc in c(1:length(x))){
  tempo<-x[[loc]]
  dl2<-ncol(tempo)-nvar
...
op<-par(mar=c(5.1, 5.1, 4.1, 2.1), mgp=c(3, 1, 0))
plot(xxyy$xx,xxyy$yy,xlab="(L+U) for sp-nvar matrix",ylab="(L+U) for sp-sp matrix",col=rgb(0,1,0,0.4),pch=19,cex=2,cex.lab=2,cex.axis=2)
